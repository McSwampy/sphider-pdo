...
function get_categories_view() {
    ...
    $result = $db->query('SELECT * FROM '.$prefix.'categories WHERE parent_num=0 ORDER BY category');
	$categories['main_list'] = $result->fetchAll();

	if (is_array($categories['main_list'])) {
		foreach ($categories['main_list'] as $_key => $_val) {
            $result = $db->query('SELECT * FROM '.$prefix.'categories WHERE parent_num='.$_val['category_id']);
...
function get_category_info($catid) {
    global $db;
	$prefix = constant('settings')['database']['table_prefix'];
	$result = $db->query("SELECT * FROM ".$prefix."categories ORDER BY category");
    $categories['main_list'] = $result->fetchAll();
	...
	if (is_array($categories['subcats'])) {
		foreach ($categories['subcats'] as $_key => $_val) {
            $result = $db->query('SELECT count(*) FROM '.$prefix.'site_category WHERE 	category_id='.(int)$_val['category_id']);
	...
	/* list category sites */
    $result = $db->query('SELECT url, title, short_desc FROM '.$prefix.'sites, '.$prefix.'site_category WHERE category_id='.$catid.' AND '.$prefix.'sites.site_id='.$prefix.'site_category.site_id order by title');
